...
| `coo-platform` | Next.js dashboard base (most recent, monorepo-shaped) | `packages/dashboard/` (base) |
...
| `paperclip-businesses` / `paperclip-operations-hub` | Paperclip integration patterns | MCP entries |
...
|---|---|
| `cold-email-agent` / `ai-integraterz-cold-email` / `autonomous-sdr-agent` / `social-sdr` / `gtm-company` | `examples/sdr-fleet/` |
...
| `coo-platform` / `coo-dashboard` / `coo-agent` (when not contributing patterns) | `examples/coo-control-room/` |
| `titans-*` (council outputs, GTM strategies) | Stay in their own repos; no migration; reference by manifest only |
| `recruiterstack` / `big-steel-agents` / `autonomous-marketing-agency` / `autonomous-job-hunter` / `paperclip-businesses` / `jay-abraham-power-partnerships` / `josh-oc-project` / `zion-minecraft-agents` / `funnel-flow-ai` / `business-os-pro` / `expert-series-v3` / `genius-tap-content-system` / `Orgo-Computer-Use-Agents` / `UAIS-*` / `ai-integraterz` | One-by-one onboard via `examples/<name>/` as scope arises; no big-bang migration |
...
- **Multiple dashboards.** Pick coo-platform's base, fold the others in. No three-dashboard monorepo.
