...
## Cursor SDK + Symphony intake

- Added `docs/symphony-intake.md` evaluating OpenAI Symphony as a coding-agent orchestration pattern, not a default dependency.
...
- Added `docs/cursor-sdk-intake.md` and `docs/builder-tool-architecture.md` to position Cursor SDK as an optional builder-swarm backend behind the same `WORKFLOW.md` harness, alongside Codex, Claude Code, OpenClaw, Hermes subagents, Composio/MCP connectors, and browser/computer tools.
...
- Added `docs/hermes-first-install-walkthrough.md` so a fresh-machine install starts by installing and verifying Hermes Agent, then configures Super Agent tools, shared brain sync, channels, tier, and smoke tests.
...
## Fresh-machine install boot-command clarification
